...
## CoreLogic Research Project Setup (2026-05-18)

[LEARN:data] `C:\CoreLogic\` is the read-only raw extracts root. NO tool call may write, delete, or modify anything under this path. All reads go through `shared_utils/R/corelogic_loader.R` (R), `shared_utils/python/corelogic_loader.py` (Python), or `shared_utils/julia/corelogic_loader.jl` (Julia). Violations break the data contract.

[LEARN:data] CoreLogic working format is Apache Arrow / Parquet, partitioned by state (and year for OT). Choice rationale: predicate pushdown means a project that wants only Ohio 2018-2024 reads ~2% of the file; columnar compression drops 25 GB → ~2 GB; duckdb can query the same parquet from R, Python, and Julia (single canonical store, three languages). One-time conversion via `shared_utils/R/convert_raw_to_parquet.R`.

[LEARN:data] The prior by-state CSV split (in `C:\CoreLogic\housing\{OwnerTransfer,PropertyCharacteristics}\by_state\`) leaked junk-state rows into bogus partition files: `corelogic_ot_A.csv`, `_AE.csv`, numeric state codes like `12011.csv`, `63.94.csv`. These are data quality issues from prior pipeline. The conversion script quarantines them to `data/corelogic_extracts/_quarantine/` rather than deleting — audit them later, never lose data.

[LEARN:workflow] Project naming convention: `projects/NN_<short_slug>/` where NN is a zero-padded two-digit number, slug is snake_case. Scaffold via `/new-project <slug>`. Auto-numbered by scanning existing folders. Status field in each project's README.md is one of: SCOPING, EXPLORATION, ANALYSIS, WRITING, REVIEW, SUBMITTED, R&R, PUBLISHED.

[LEARN:workflow] Every project R script starts with `source(here::here("projects/NN_<slug>/scripts/R/00_setup.R"))`. The setup file is the ONLY place that touches paths — sources shared_utils, sets `data_dir`, `out_dir`, `project_dir`. No `setwd()` anywhere. No hardcoded paths anywhere else. This is the discipline that makes scripts portable across machines and across projects.

[LEARN:workflow] Sample-data dev pattern: `data/corelogic_extracts/ot_sample_10k.parquet` + `prop_sample_10k.parquet` for fast iteration. Every loader call accepts `sample = TRUE` to read these instead of the full partitioned store. Promotes to full data with a one-line flip when ready. Saves ~20-minute reads during exploration.

[LEARN:workflow] Prior Ohio cleaned + geocoded files (from `C:\CoreLogic\housing\*.csv` and `.dta`) are wrapped as baseline inputs at `data/corelogic_baseline/`, NOT rebuilt. `PROVENANCE.md` documents what was done (in Stata, by user, pre-this-repo) honestly. They're not reproducible from this repo. For new analyses, prefer the from-raw pipeline in shared_utils/. The baseline files are for quick replication of prior work or as a sanity check.

[LEARN:meta] Repository transitioned from template-development mode to user-project mode on 2026-05-18. Session logs now DO go in `quality_reports/session_logs/` (the prior `[LEARN:meta]` exception for template-meta-work no longer applies). Plans and specs continue to live in `quality_reports/{plans,specs}/`.
