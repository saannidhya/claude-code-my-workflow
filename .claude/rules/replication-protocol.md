...
## Per-Project Replication Packages

On submission (status: `SUBMITTED`), build a self-contained replication package under `projects/NN_<slug>/replication/`:

```
projects/NN_<slug>/replication/
├── README.md           Step-by-step replication instructions
├── data/               Symlinks or copies of input data (or a download script)
├── scripts/            Copies of scripts/ at submission SHA
├── outputs/            Expected outputs (for diff-checking on replication)
└── env/                renv.lock + uv.lock snapshot at submission
```

Generated by a future skill (`/build-replication-package <project>`). For now, build manually before submitting.

Critical: replication packages must NOT contain CoreLogic raw data — only derived data the user has license to redistribute, or scripts that re-derive from raw given the user's own CoreLogic access.
